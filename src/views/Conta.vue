<template>
  <q-page-container>
    <div class="flex row">
      <q-card style="flex: 1" class="flex column items-center q-ma-md">
        <q-card-section class="flex items-center column">
          <div class="text-h6">Dados do Usuario</div>
          <q-avatar color="primary" round text-color="white" size="72px">{{username.split(' ').map(name => name[0]).join('').toUpperCase()}}</q-avatar>
        </q-card-section>
        <q-form
            @submit="atualizarCadastro"
        >
          ...
          <div class="flex-center flex">
            <q-btn label="Atualizar dados" type="submit" color="blue-grey-10"/>
          </div>
        </q-form>
      </q-card>
      <q-card style="flex: 1" class="flex items-center column  q-ma-md">
        <q-card-section class="flex items-center column">
          <div class="text-h6">Dados de Pagamento</div>
        ...
        >
          ...
          <div class="flex flex-center q-pa-sm">
            <q-btn label="Atualizar dados de pagamento" type="submit" color="blue-grey-10"/>
          </div>
        </q-form>
      </q-card>
      <q-card style="flex: 1" class="flex column items-center q-ma-md">
        <q-card-section>
          <div class="text-h6">Dados do Plano</div>
        </q-card-section>
        <q-carousel
            v-model="card.plano"
        ...
        <q-card-actions>
          <q-btn label="Atualizar Plano" @click="atualizarPlano()" color="blue-grey-10"/>
        </q-card-actions>
      </q-card>
    </div>
  </q-page-container>
...
export default class Conta extends Vue {
  ...
  mounted() {
    ...
    if (localStorage.getItem('cadastro')) {
      this.retrievedObjectConta = localStorage.getItem('cadastro');
      this.dadosConta = JSON.parse(this.retrievedObjectConta);
      this.username = this.dadosConta.name
      this.cadastro = this.dadosConta
    }
